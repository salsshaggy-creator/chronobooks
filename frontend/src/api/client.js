...
export const api = {
  ...
  getCustomerStatement: (id, params = {}) => request(`/customers/${id}/statement${params.from || params.to ? `?${new URLSearchParams(params)}` : ''}`),
  ...
  getSupplierStatement: (id, params = {}) => request(`/suppliers/${id}/statement${params.from || params.to ? `?${new URLSearchParams(params)}` : ''}`),
  ...
  voidBill: (id) => request(`/bills/${id}/void`, { method: 'POST' }),
  ...
  cashFlow: (from, to) => request(`/reports/cash-flow?from=${from}&to=${to}`),
  ...
  listPayableFromAccounts: () => request('/accounting/payable-from-accounts'),
  ...
  voidJournalEntry: (entryId) => request(`/accounting/journal-entries/${entryId}/void`, { method: 'POST' }),
