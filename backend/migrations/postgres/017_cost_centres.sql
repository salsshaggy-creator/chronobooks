...
-- income and expenses can be broken down by centre. cost_centres.id is SERIAL/INTEGER
-- (migration 008) so this FK column must be INTEGER, not UUID -- see the
-- budgets.account_id lesson from migration 015. Nullable so every existing row and
-- every company not using this module (the default) is completely unaffected.
